                              ...
                              dropdownButton(label = "Settings", circle = FALSE, status = "primary", size = "sm", icon = icon("gear"), tooltip = FALSE,
                                             ...
                                             fluidRow(
                                               ...
                                               column(4,
                                                      ...
                                                      sliderInputReg(inputId="yrs_vmwsc1", label = "Nb Years to Scale (1) ", value = 3, min=.25, max=3, step=.25, sep=NULL, post = " year(s)"),
                                                      sliderInputReg(inputId="vmwsc_cov1", label = "New Coverage (1)", value = 90, min=0, max=100,step=1, post = " %")
                                                ),
                                               column(4,
                                                      h4("Scale up/down (2)"),
                                                      sliderInputReg(inputId="vmwsc_start2", label = "Year to Start (2)", value = 2023, min=2019, max=2030,step=1, sep=NULL),
                                                      sliderInputReg(inputId="yrs_vmwsc2", label = "Nb Years to Scale(2) ", value = 3, min=.25, max=3, step=.25, sep=NULL, post = " year(s)"),
                                                      sliderInputReg(inputId="vmwsc_cov2", label = "New Coverage (2)", value = 90, min=0, max=100,step=1, post = " %")
                                               )
                                             )
                              )
             )
