  ...
  return(
    tibble(times = startyear + outoderun[, 1], 
      ...
      mutate(year = floor(times), 
             trtf = hisf + vmwf + severef + rcdf + totMS_predf_ode + SMCtrt_predf + totIPTp_predf , 
             reported = (input$report/100 * hisf) + (input$reportv/100 * vmwf) + severef + rcdf + totMS_predf_ode + SMCtrt_predf + totIPTp_predf, 
             package = package)
  )
}