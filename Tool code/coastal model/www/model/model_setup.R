  ...
  # with(reactiveValuesToList(input_UI),
       with(input_UI,
       {
         ...
         for (n in 1:N){
           ...
           seas <- (48.54206007 + amp1*cos(2*pi*t_internal + phi1) + -48.71330976 + amp2*cos(4*pi*t_internal - phi2))^pe
  ...
  with(as.list(c( parpro)),
       {
         ...
         for (n in 1:N){
           trt_predf[,n]<-rowSums(tran[,c(traind[c(15:17,28:30,33:35,39:41,46:48,53,56,59, 60:62, 78:80,85:87, 93:95 ),n])])/12
         }
         ...
         for (n in 1:N){
           totclininc_predf[,n]<-rowSums(tran[,c(traind[c(14:17,27:35,38:41,45:48,52,53,55,56,58,59,  60:62, 78:80,85:87, 93:95  ),n])])/12
         }
