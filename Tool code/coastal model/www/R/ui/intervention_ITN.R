                              ...
                              dropdownButton(label = "Settings", circle = FALSE, status = "primary", size = 'sm', icon = icon("gear"), 
                                             ...
                                             fluidRow(
                                               ...
                                               column(4,
                                                      ...
                                                      sliderInputReg(inputId="yrs_itnsc1", label = "Nb Years to Scale (1)", value = 5, min=0, max=5, step=0.25, sep=NULL, post = " year(s)"),
                                                      sliderInputReg(inputId="itnsc_cov1", label = "New Coverage (1)", value = 90, min=0, max=100,step=1, post = " %"),
                                                      sliderInputReg(inputId="itnsc_eff1", label = "New Effectiveness (usage and efficacy) (1)", value = 50, min=0, max=100,step=1, post = " %")
                                               ),
                                               column(4,
                                                      h4("Scale up/down (2)"),
                                                      sliderInputReg(inputId="itnsc_start2", label = "Year to Start (2)", value = 2025, min=2019, max=2030,step=1, sep=NULL),
                                                      sliderInputReg(inputId="yrs_itnsc2", label = "Nb Years to Scale (2)", value = 5, min=0, max=5, step=0.25, sep=NULL, post = " year(s)"),
                                                      sliderInputReg(inputId="itnsc_cov2", label = "New Coverage (2)", value = 90, min=0, max=100,step=1,  post = " %"),
                                                      sliderInputReg(inputId="itnsc_eff2", label = "New Effectiveness (usage and efficacy) (2)", value = 50, min=0, max=100,step=1, post = " %")
                                               )
                                             )
                              )
             )
