             ...
             conditionalPanel(condition = "input.smcsc_switch",
                              dropdownButton(label = "Settings", circle = FALSE, status = "primary", size = 'sm', icon = icon("gear"), 
                                             ...
                                             fluidRow(
                                               ...
                                               column(4,
                                                      h4("Scale up/down (2)"),
                                                      sliderInputReg(inputId="yr_smc_sc2", label = "Year to start scale up/down (2)", value = 2025, min=2019, max=2030,step=0.5, sep=NULL),
                                                      sliderInputReg(inputId="smc_covsc2", label = "New coverage of SMC programme (2)", value = 20, min=0, max=100,step=1, post = " %")
                                               )
                                             )
                              )
             )
