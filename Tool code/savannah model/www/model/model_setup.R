...
malrates <- function(x, nun_sens, input_UI, t, ti, scenario) {
  # with(reactiveValuesToList(input_UI),
       with(input_UI,
       {
         ...
         # itn_cov = itn_cov/100
         itn_cov12 = itn_dis12/100#itn_dis12/initP*net_multiplier
         itn_cov13 = itn_dis13/100#itn_dis13/initP*net_multiplier
         itn_cov14 = itn_dis14/100#itn_dis14/initP*net_multiplier
         itn_cov15 = itn_dis15/100#itn_dis15/initP*net_multiplier
         itn_cov16 = itn_dis16/100#itn_dis16/initP*net_multiplier
         itn_cov17 = itn_dis17/100#itn_dis17/initP*net_multiplier
         itn_cov18 = itn_dis18/100#itn_dis18/initP*net_multiplier
         ...
         muC <- ((t_internal<2013)*muC12 + 
                (t_internal>=2013)*(t_internal<2014)*muC13 +
                (t_internal>=2014)*(t_internal<2015)*muC14 +
                (t_internal>=2015)*(t_internal<2016)*muC15 +
                (t_internal>=2016)*(t_internal<2017)*muC16 +
                (t_internal>=2017)*(t_internal<2018)*muC17 +
                (t_internal>=2018)*muC18) /1000                    #imported rate per pop
         ...
         covRCDi=1/100*((t_internal<2013)*acd_cov12 + 
                    (t_internal>=2013)*(t_internal<2014)*acd_cov13 +
                    (t_internal>=2014)*(t_internal<2015)*acd_cov14 +
                    (t_internal>=2015)*(t_internal<2016)*acd_cov15 +
                    (t_internal>=2016)*(t_internal<2017)*acd_cov16 +
                    (t_internal>=2017)*(t_internal<2018)*acd_cov17 +
                    (t_internal>=2018)*(1-rcdsc_switch)*acd_cov18 +
                    rcdsc_switch*((t_internal>=2018)*(t_internal<yr_rcdscstart)*acd_cov18 +
                                    (t_internal>=yr_rcdscstart)*acdsc_cov1)) 
       
         
           pcd=1/100*((t_internal<2013)*pcd_cov12 + 
                          (t_internal>=2013)*(t_internal<2014)*pcd_cov13 +
                          (t_internal>=2014)*(t_internal<2015)*pcd_cov14 +
                          (t_internal>=2015)*(t_internal<2016)*pcd_cov15 +
                          (t_internal>=2016)*(t_internal<2017)*pcd_cov16 +
                          (t_internal>=2017)*(t_internal<2018)*pcd_cov17 +
                          (t_internal>=2018)*(1-rcdMS_switch)*pcd_cov18 +
                        rcdMS_switch*((t_internal>=2018)*(t_internal<yr_rcdMSstart)*pcd_cov18 +
                                          (t_internal>=yr_rcdMSstart)*pcdsc_cov1)) 
         ...
         itn<-(t_internal>=itn_start)*(t_internal< (itn_start+yrs_itnsc0))*(t_internal*(itn_cov12)/yrs_itnsc0 -(itn_cov12)/yrs_itnsc0*itn_start)*itn_aversion+(t_internal>=(itn_start+yrs_itnsc0))*itn_cov12*itn_aversion +
           ...
           (t_internal>=2018)*(1-itnsc_switch)*itn_cov18*itn_aversion  + 
           itnsc_switch*((t_internal>=2018)*(t_internal<itnsc_start1)*itn_cov18*itn_aversion +
                           ...
                           (t_internal>=(itnsc_start1+yrs_itnsc1))*(t_internal<(itnsc_start2))*itnsc_cov1*itnsc_eff1 +
                           ...
                           (t_internal>=(itnsc_start2+yrs_itnsc2))*itnsc_cov2*itnsc_eff2
           )
         ...
         for (n in 1:N){
           #seas<-1+amp1*cos(2*pi*(t_internal-phi))^pe # seasonal forcing signal
          
           seas <- (amp1 + amp2*cos(2*pi*t_internal - (phi + 8)))^pe # seasonal forcing signal
          ...
          # clus_func <- (((incf)/mvis)<initP/ss)*clustRCDcoex*ss/initP*((incf)/mvis)+clustRCDcoex
           ...
           clustRCDcoex <- (1-rcdsc_switch)*clustRCDcoex + rcdsc_switch*clustRCDcoex2
           ss <- (1-rcdsc_switch)*ss + rcdsc_switch*ss2
           rateRCD<- min(incf*(1+clustRCDcoex)*ss/popf[n],1)
           tauRCD<- rateRCD
           
           no_visits<-incf
           sample_rcd<-min((incf)*ss,popf)
           ...
           covSMC<-(t_internal>=(2012+mon_smc))*(t_internal<(2012+mon_smc+1/12))*smc_cov12  +
             ...
             (t_internal>=(2018+mon_smc))*(t_internal<(2018+mon_smc+1/12))*smc_cov18  + 
             (1-scenario[12])*(t_internal>=2019)*(t_internal>=(floor(t_internal)+mon_smc))*(t_internal<(floor(t_internal)+mon_smc+1/12))*smc_cov18 +
  ...
  with(as.list(c( parpro)),
       {
         ...
         for (n in 1:N){
           ...
           totrcd_predf[,n]<-rowSums(tran[,c(traind[c(60:62),n])])/12   #those identified through RCD Pf (In/a/c -> Th)
           totrcd_visits[,n]<-tran[,c(traind[c(76),n])]/12   #visits made during RCD
           totrcd_sample[,n]<-tran[,c(traind[c(77),n])]/12   #sample assessed during RCD
           totMS_predf[,n]<-rowSums(tran[,c(traind[c(78:80),n])])/12   #those identified through MS Pf (In/a/c -> Th)
           totMS_sample[,n]<-tran[,c(traind[c(81),n])]/12   #sample assessed during MS
