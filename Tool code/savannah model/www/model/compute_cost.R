  ...
  for (i in 1:100) {
    ...
    costs <- simul_epi %>%
      mutate(
        ...
        cost_acd = input_sample$cpdrugfov * rcdf + input_sample$cpacdvisit * rcdvisits + input_sample$cprdt * rcdsample,
        cost_ms =  input_sample$cpdrugfov * totMS_predf_ode + input_sample$costMS * totMS_sample_ode + input_sample$cprdt * totMS_sample_ode
    ...
    if(package != "Baseline"){
      ...
      # ACD
      if(input_sample$rcdsc_switch){
        period <- ((input_sample$yr_rcdscstart - startyear) * 12 + 1):tmonths
        
        costs$cost_acd[period] <- 
          input_sample$cpdrugfov * simul_epi$rcdf[period] + # drugs
          input_sample$cpacdvisit1 * simul_epi$rcdvisits[period] + # cost per vist 
          input_sample$cprdt * simul_epi$rcdsample[period]  # diagnostics for sample per case investigated
      }
      
      # AMS
      if(input_sample$rcdMS_switch){
        ...
        costs$cost_ms[period] <- 
          ...
          input_sample$costMS2 * simul_epi$totMS_sample_ode[period] + # cost per person screened
